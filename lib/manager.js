    ...
    worker.on('message', (m) => {
      switch (m.action) {
        case 'complete':
          ...
          if (stats.first && stats.iterations > 1) {
            for (let i = 0; i < stats.iterations; i++) {
              self._createJob(conf);
            }
          } else {
            self._results.set(conf._id, m.data);
            stats.completed++;
          }
          ...
          if (self._jobComplete(conf._id)) {
            self.emit('result', self._results.get(conf._id));
          }
        ...
        case 'error':
          ...
          if (conf._retry < this.config.maxRetries) {
            ...
            log.info(`Failed all retries for ${conf.testPath}. Moving on.`);
